...
<div class="dashboard-container" id="dashboardContainer">
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a class="nav-link{{ request()->routeIs('super-admin.pending-landlords') ? ' active' : '' }}" href="{{ route('super-admin.pending-landlords') }}">
                <span class="nav-icon"><i class="fas fa-user-clock"></i></span> <span class="nav-label">Pending Approvals</span>
            </a>
            <a class="nav-link{{ request()->routeIs('super-admin.users') ? ' active' : '' }}" href="{{ route('super-admin.users') }}">
                <span class="nav-icon"><i class="fas fa-users"></i></span> <span class="nav-label">User Management</span>
            </a>
            <a class="nav-link{{ request()->routeIs('super-admin.apartments') ? ' active' : '' }}" href="{{ route('super-admin.apartments') }}">
                <span class="nav-icon"><i class="fas fa-building"></i></span> <span class="nav-label">Properties</span>
            </a>
...
<script>
    document.getElementById('sidebarCollapseBtn').onclick = function() {
        ...
        if (cont) {
            ...
            try { localStorage.setItem('superAdminSidebarCollapsed', cont.classList.contains('collapsed') ? '1' : '0'); } catch (e) {}
        }
    };
    (function() {
        var cont = document.getElementById('dashboardContainer');
        try { if (localStorage.getItem('superAdminSidebarCollapsed') === '1') cont.classList.add('collapsed'); } catch (e) {}
    })();
</script>
</body>
</html> 